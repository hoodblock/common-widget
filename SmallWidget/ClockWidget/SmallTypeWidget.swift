...
struct SmallTypeProvider: IntentTimelineProvider {
    
    // 添加组件弹框预览图
    func placeholder(in context: Context) -> SmallTypeEntry {
        ...
        return SmallTypeEntry(date: Date(), configuration: SmallWidgetConfigurationIntent())
    }
    
    func getSnapshot(for configuration: SmallWidgetConfigurationIntent, in context: Context, completion: @escaping (SmallTypeEntry) -> ()) {
        let entry = SmallTypeEntry(date: Date(), configuration: configuration)
        completion(entry)
    }
    
    func getTimeline(for configuration: SmallWidgetConfigurationIntent, in context: Context, completion: @escaping (Timeline<Entry>) -> ()) {
        
        // 时间线数组. entries提供下次更新的数据
        var entries: [SmallTypeEntry] = []
        
        let currentDate = Date()
            
        for offset in 0 ..< 60 {
            let entryDate = Calendar.current.date(byAdding: .second, value: offset, to: currentDate)!
            let entry = SmallTypeEntry(date: entryDate, configuration: configuration)
            entries.append(entry)
        }
        // policy 时机 刷新数据和控制下一步刷新时间,时间线结束刷新一次，一分钟刷新一次
...
// TimelineEntry 基础元素
struct SmallTypeEntry: TimelineEntry {
    let date: Date
...
struct SmallTypeWidgetEntryView : View {
    ...
    var body: some View {
        ...
        .frame(minWidth: (UserDefaults.standard.value(forKey: small_width) as? CGFloat), minHeight: (UserDefaults.standard.value(forKey: small_height) as? CGFloat))
        .onAppear(perform: {
            // 设置背景为小组件截图的背景
            // 设置背景透明，并且设置背景为截图背景
            ...
            if (entry.configuration.dynamicTransparentType?.dynamicTransparent != dynamic_follow_app) && (entry.configuration.dynamicTransparentType?.dynamicTransparent?.count ?? 0 > 0) {
                ...
                if entry.configuration.dynamicTransparentType?.dynamicTransparent == dynamic_left_top {
                    if let croppedCGImage = cropImage(image: transparentTypeImage, rect: CGRect(origin: ClockWidgetBundle().getSmallWidgetPos(.topLeft), size: ClockWidgetBundle().getWidgetSize(.small))) {
                        transparentTypeImage = UIImage(cgImage: croppedCGImage)
                    }
                } else if entry.configuration.dynamicTransparentType?.dynamicTransparent == dynamic_right_top {
                    if let croppedCGImage = cropImage(image: transparentTypeImage, rect: CGRect(origin: ClockWidgetBundle().getSmallWidgetPos(.topRight), size: ClockWidgetBundle().getWidgetSize(.small))) {
                        transparentTypeImage = UIImage(cgImage: croppedCGImage)
                    }
                } else if entry.configuration.dynamicTransparentType?.dynamicTransparent == dynamic_left_medium {
                    if let croppedCGImage = cropImage(image: transparentTypeImage, rect: CGRect(origin: ClockWidgetBundle().getSmallWidgetPos(.middleLeft), size: ClockWidgetBundle().getWidgetSize(.small))) {
                        transparentTypeImage = UIImage(cgImage: croppedCGImage)
                    }
                } else if entry.configuration.dynamicTransparentType?.dynamicTransparent == dynamic_right_medium {
                    if let croppedCGImage = cropImage(image: transparentTypeImage, rect: CGRect(origin: ClockWidgetBundle().getSmallWidgetPos(.middleRight), size: ClockWidgetBundle().getWidgetSize(.small))) {
                        transparentTypeImage = UIImage(cgImage: croppedCGImage)
                    }
                } else if entry.configuration.dynamicTransparentType?.dynamicTransparent == dynamic_left_bottom {
                    if let croppedCGImage = cropImage(image: transparentTypeImage, rect: CGRect(origin: ClockWidgetBundle().getSmallWidgetPos(.bottomLeft), size: ClockWidgetBundle().getWidgetSize(.small))) {
                        transparentTypeImage = UIImage(cgImage: croppedCGImage)
                    }
                } else if entry.configuration.dynamicTransparentType?.dynamicTransparent == dynamic_right_bottom {
                    if let croppedCGImage = cropImage(image: transparentTypeImage, rect: CGRect(origin: ClockWidgetBundle().getSmallWidgetPos(.bottomRight), size: ClockWidgetBundle().getWidgetSize(.small))) {
                        transparentTypeImage = UIImage(cgImage: croppedCGImage)
                    }
                }
...
struct SmallTypeWidget: Widget {
    
    let kind: String = "SmallTypeWidget"
    
    var body: some WidgetConfiguration {
        if #available(iOSApplicationExtension 15.0, *) {
            return IntentConfiguration(kind: kind, intent: SmallWidgetConfigurationIntent.self, provider: SmallTypeProvider()) { entry in
                SmallTypeWidgetEntryView(entry: entry)
            }
            .supportedFamilies([.systemSmall])
            .configurationDisplayName("Small Widgets")
...
struct SmallTypeWidget_Previews: PreviewProvider {
    static var previews: some View {
        SmallTypeWidgetEntryView(entry: SmallTypeEntry(date: Date(), configuration: SmallWidgetConfigurationIntent()))
