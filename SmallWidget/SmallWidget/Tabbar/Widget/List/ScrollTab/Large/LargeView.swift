...
struct LargeView: View {
    ...
    let classSelf: [Any] = [
        LargeStashView_0.self
    ]
    
    var body: some View {
        GeometryReader { proxy in
            let leftWidth = (UIScreen.main.bounds.width - ViewLayout.S_W_20() * 2 - ViewLayout.S_W_15()) / 2
            ScrollView(.vertical, showsIndicators: false) {
                VStack (alignment: .center, spacing: 0) {
                    // small
                    LazyVGrid(columns: [GridItem(.fixed(leftWidth), spacing: ViewLayout.S_W_15()), GridItem(.fixed(leftWidth), spacing: ViewLayout.S_W_15())], spacing: 0) {
